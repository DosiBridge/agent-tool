...
async def get_llm_config(
    ...
    try:
        ...
        if not config:
            raise HTTPException(
                status_code=404,
                detail="No LLM configuration found. Please configure an LLM provider via environment variables or create a personal LLM config."
            )
...
async def set_llm_config(
    ...
    try:
        ...
        # Users cannot set is_default=True (only system can set defaults from env vars)
...
async def list_llm_configs(
    ...
    - Users see ONLY global configs that are BOTH active AND default (active=True AND is_default=True)
    ...
    try:
        ...
        # Build query: user's own configs + active default global configs
...
async def update_llm_config(
    ...
    try:
        ...
        # Prevent editing default LLM configurations (only system can manage defaults from env vars)
        if llm_config.is_default:
            raise HTTPException(
                status_code=403,
                detail="Cannot edit default LLM configuration. Default configurations are managed by the system."
            )

        # Users cannot change is_default status - only system can set defaults from env vars
...
async def switch_llm_config(
    ...
    - Users can only switch to global configs that are BOTH active AND default (active=True AND is_default=True)
    """
    try:
        user_id = current_user.id if current_user else None
        ...
        # Users can only switch to global configs that are BOTH active AND default
        # Global configs use user_id=None (backward compatible with user_id=1)
        llm_config = db.query(LLMConfig).filter(
            LLMConfig.id == config_id,
            or_(
