...
export default function SettingsPanel() {
  ...
  const handleDeleteServer = async (name: string) => {
    ...
    if (isGlobal) {
      toast.error("Cannot delete global servers. Global servers are managed by the system.");
      return;
    }
  ...
  const handleToggleServer = async (name: string) => {
    ...
    if (isGlobal) {
      toast.error("Cannot toggle global servers. Global servers are managed by the system.");
      return;
    }
  ...
  const startEditing = (server: MCPServer) => {
    ...
    if (isGlobal) {
      toast.error("Cannot edit global servers. Global servers are managed by the system.");
      return;
    }
  ...
  const startEditingLLMConfig = (config: LLMConfigListItem) => {
    ...
    if (isGlobal) {
      toast.error("Cannot edit global LLM configurations. Global configs are managed by the system.");
      return;
    }
  ...
  const handleDeleteConfig = async (configId: number) => {
    ...
    if (isGlobal) {
      toast.error("Cannot delete global LLM configurations. Global configs are managed by the system.");
      return;
    }
                            ...
                            <div className="flex items-start justify-between">
                                ...
                                <div className="flex items-center gap-4 text-xs text-zinc-500">
                                  ...
                                  {isGlobal && (
                                    <p className="text-xs text-blue-400 mt-2 flex items-center gap-1">
                                      <Lock className="w-3 h-3" />
                                      Read-only: Global configs are managed by the system
                                    </p>
                                  )}
                              </div>
                              <div className="flex items-center gap-2 ml-4">
                                ...
                                {!config.active && (
                                  ...
                                  {!canModify && (
                                    <div className="p-2 text-zinc-500" title="Global configs are managed by the system">
                            ...
                            <div className="flex items-center gap-2 text-xs text-zinc-500">
                              ...
                              {isGlobal && (
                                <p className="text-xs text-blue-400 mt-1 flex items-center gap-1">
                                  <Lock className="w-3 h-3" />
                                  Read-only: Global servers are managed by the system
                                </p>
                              )}
                          </div>
                        </div>
                            ...
                            {deletingServerName === server.name ? (
                              ...
                              <>
                                ...
                                {isGlobal && (server.user_id === null || server.user_id === undefined || server.user_id === 1) && server.id ? (
                                  ...
                                  <div className="p-2 text-zinc-500" title="Global servers are managed by the system">
                                    <Lock className="w-4 h-4" />
                                  </div>
                                )}
                              </>
