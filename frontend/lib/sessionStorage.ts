 ...
 */
export function saveStoredMessages(sessionId: string, messages: StoredMessage[]): void {
  if (typeof window === 'undefined') return;
    ...
    if (sessionIndex >= 0) {
      ...
      if (!sessions[sessionIndex].firstMessage && messages.length > 0) {
        ...
        if (firstUserMessage) {
          ...
          sessions[sessionIndex].title = firstUserMessage.content.slice(0, 50) || 'New Chat';
        }
      }
 ...
 */
export function createStoredSession(sessionId: string, title?: string): StoredSession {
 ...
 */
export function updateStoredSessionTitle(sessionId: string, title: string): void {
  const sessions = getStoredSessions();
  const sessionIndex = sessions.findIndex(s => s.id === sessionId);
  if (sessionIndex >= 0) {
...
export function deleteStoredSession(sessionId: string): void {
  ...
  try {
    ...
    console.error(`Failed to delete messages for session ${sessionId}:`, error);
  }
